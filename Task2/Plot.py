...
def Lwr(idx, tau):
    # Px = np.array(np.linspace(min(X), max(X), 00))
    Px = X
    theta = np.array([Lwr_Single(x, tau) for x in Px])
    Py = np.array(list(map(lambda x, y: x*y, theta, Px)))
    Pt = np.array(sorted(list(map(lambda x, y: (x, y), Px, Py))))
